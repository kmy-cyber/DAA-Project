...
Equivalente: resolver el \textit{Minimum Degree-Constrained Spanning Tree}.
...
A este problema lo llamamos \textsc{DC-MST-Decision} (Degree-Constrained Minimum Spanning Tree Decision).

\subsection{Pertenencia a NP}

Dado un conjunto de aristas $T$, puede verificarse en tiempo polinomial que:
...
Por lo tanto, \textsc{DC-MST-Decision} pertenece a NP.

\subsection{Demostración de NP-completitud}

Para demostrar que \textsc{DC-MST-Decision} es NP-completo, se presenta una 
...
de \textsc{DC-MST-Decision} de la siguiente manera:
...
Por lo tanto, constituye un conjunto $T$ válido para \textsc{DC-MST-Decision} 
con costo total $|V|-1 \le K$.

\paragraph{($\Leftarrow$)}  
Si existe un conjunto $T$ que satisface las restricciones de 
\textsc{DC-MST-Decision}, entonces $T$ es un árbol con $|V|-1$ aristas y 
$\deg_{G_T}(v) \le 2$ para todo $v$.  
